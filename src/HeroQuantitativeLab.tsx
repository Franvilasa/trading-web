...
export default function HeroQuantitativeLab() {
  ...
  // --- VARIABLES DE CONTROL (igual que antes) ---
  let tiempo = 400;
  ...
  // --- NUEVA FUNCIÓN DE PROYECCIÓN 3D (efecto Star Wars) ---
  // Convierte (x, y) en coordenadas de pantalla con perspectiva.
  // Cuanto más a la derecha (x grande), más cerca y más grande se ve.
...
const proyectar = (x: number, y: number, ancho: number, alto: number) => {
  ...
  // --- RENDERIZADO PRINCIPAL (modificado para usar proyectar) ---
  const renderizar = () => {
    tiempo += 1;
    ...
    for (let i = 0; i < puntosPIB.length; i++) {
      ...
      ctx.lineWidth = 2.0 * escala; // el trazo se engrosa al acercarse
    ...
    for (let i = 10; i < puntosActivo.length; i += 75) {
      ...
      if (p.x > ancho * 0.38 && p.x < ancho - 40) {
        ...
        const tamaño = 6 * escala; // crecen al acercarse
    ...
    // 6. MÁSCARA DE DEGRADADO (la que tapa la izquierda)
    ...
    // 7. VELAS JAPONESAS (por encima de la máscara, con perspectiva)
